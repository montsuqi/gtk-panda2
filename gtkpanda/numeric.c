	...
	Numeric	num)
{
	...
	if (NUMERIC_IS_NAN(num))
		fprintf(stderr, _("Cannot convert NaN to int4"));
	...
	NumericVar *dest)
{
	...
	while	(*cp)	{
		...
		if (*cp == '.')	{
			if (have_dp)
				fprintf(stderr, _("Bad numeric input format '%s'"), str);
			have_dp = TRUE;
			cp++;
		} else
		if		(*cp == 'e' || *cp == 'E')
	...
	int			scale)
{
	...
	if		(  var->weight  >=  maxweight  )	{
		...
		if (tweight >= maxweight && i < var->ndigits)
			fprintf(stderr, _("overflow on numeric \nABS(value) >= 10^%d for field with precision %d scale %d"),
	...
	NumericVar *result)
{
	...
	if (ndigits_tmp == 1)
		fprintf(stderr, _("division by zero on numeric"));
	...
	NumericVar *result)
{
	...
	if (stat < 0)
		fprintf(stderr, _("math error on numeric - cannot compute SQRT of negative value"));
	...
	NumericVar	*result)
{
	...
	for	( i = x.weight, d = 0; i >= 0; i--, d++ )	{
		...
		if (global_rscale >= 1000)
			fprintf(stderr, _("argument for EXP() too big"));
	}
	...
	NumericVar *result)
{
	...
	if (CmpVar(arg, &const_zero) <= 0)
		fprintf(stderr, _("math error on numeric - cannot compute LN of value <= zero"));
