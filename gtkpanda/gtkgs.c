...
gtk_gs_init (GtkGS *gs)
{
  ...
  gs->antialiased = TRUE;
  gs->override_size = FALSE;
  gs->default_size = 8;
  gs->override_orientation = FALSE;
  gs->respect_eof = TRUE;
  gs->show_scroll_rect = TRUE;
  gs->fallback_orientation = 0;
  gs->zoom_factor = 1.0;
  gs->scroll_step = 0.25;
...
#define NUM_ALPHA_ARGS 10
  ...
  /* set up the args... */
  gs_args = g_strsplit(GS_PATH,
           " ", NUM_GS_ARGS);
  for(i = 0; i < NUM_GS_ARGS && gs_args[i]; i++, argc++)
    argv[argc] = gs_args[i];

  if(gs->antialiased) {
    if(strlen(ALPHA_PARAMS) == 0)
...
check_pdf (GtkGS *gs)
{
  ...
  if ((file = fopen (filename, "r"))
    ...
    cmd = g_strdup_printf (GS_PATH
      " -q -dNODISPLAY -dSAFER -dDELAYSAFER"
      " -sDSCname=%s -sPDFname=%s pdf2dsc.ps"
      " -c quit",
               filename_dsc, fname);
    g_free (fname);
...
gtk_gs_new (GtkAdjustment *hadj, GtkAdjustment *vadj)
{
  ...
  if(NULL == hadj)
    hadj = GTK_ADJUSTMENT(gtk_adjustment_new(0.0, 0.0, 1.0,
               0.01, 0.1, 0.09));
  if(NULL == vadj)
    vadj = GTK_ADJUSTMENT(gtk_adjustment_new(0.0, 0.0, 1.0,
               0.01, 0.1, 0.09));
...
gtk_gs_load (GtkGS *gs, const gchar *fname)
{
  ...
  if (fname == NULL) {
    if(gs->pstarget != NULL &&
       gdk_window_is_visible(gs->pstarget))
      gdk_window_hide(gs->pstarget);
    gs->gs_status = "";
    return FALSE;
  }
  ...
  (gs->gs_psfile = fopen (filename, "r")) == NULL) {
    ...
    gs->doc = psscan(gs->gs_psfile, gs->respect_eof, fname);
...
gtk_gs_set_page_size(GtkGS *gs, gint new_pagesize, gint pageid)
{
  ...
  gint new_urx = 0;
  gint new_ury = 0;
  GtkGSPaperSize *papersizes = ggv_paper_sizes;
...
gtk_gs_zoom_to_fit(GtkGS *gs, gboolean fit_width)
{
  ...
  avail_w = gs->widget.allocation.width;
  avail_h = gs->widget.allocation.height;
